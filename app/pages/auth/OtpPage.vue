...
function checkComplete() {
    isComplete.value =
        values.value.join('').length === OTP_LENGTH &&
...
<template>
    <div class="otp-page-wrapper">
        ...
        <div class="otp-form-wrapper" :class="{ shake: isError }">
            <input v-for="(val, i) in values" :key="i" ref="inputs" v-model="values[i]" maxlength="1"
                inputmode="numeric" class="otp-input" @input="onInput($event, i)" @keydown="onKeydown($event, i)"
                @paste="onPaste" />
        </div>

        <div class="confirm-btn-wrapper">
            <BaseButton variant="primary" size="sm" :disabled="!isComplete" @click="handleSubmit">
                Confirm
            </BaseButton>
